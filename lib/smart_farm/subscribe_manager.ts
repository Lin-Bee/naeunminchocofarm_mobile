...
function _subscribe(
  farmUuid: string,
  subscribers: SettingsSubscribers | StatusSubscribers,
  callback: SettingsCallback | StatusCallback
) {
...
export function subscribeFarmSettings(
  ...
  return [
    () => _unsubscribe(farmUuid, settingsSubscribers, callback),
    (newSettings: any) => _sendUpdateSettings(farmUuid, newSettings),
  ];
}
